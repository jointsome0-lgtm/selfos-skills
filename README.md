...
| `research` | `0.1.0` | Requires a host with a background subagent mechanism; without one, run the same investigation inline. Committing findings on a research branch requires git; a ticket context pointer requires write access to the driving tracker. Cited web sources require network access. No OS constraint; no other external integration. |
