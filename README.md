...
| `grill-sdd` | `0.1.5` | Requires Python 3.9+ for bundled SDD helpers and read access to the target repository. No OS constraint or required network; repository write access and external issue-tracker integration are needed only to land owner-confirmed outcomes. |
...
| `research` | `0.1.0` | Requires a host with a background subagent mechanism; without one, run the same investigation inline. Ticket-driven capture requires git worktree support and push access to the repository remote (without a writable remote the findings branch stays local and the pointer says so), plus write access to the driving tracker. Cited web sources require network access. No OS constraint; no other external integration. |
...
| `slice` | `0.1.5` | Requires Python 3.9+ for bundled SDD helpers, read access to the target repository, network access, and authenticated GitHub issue read/write integration to publish confirmed tickets. No OS constraint. |
