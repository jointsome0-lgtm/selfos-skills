...
| `improve-codebase-architecture` | `0.1.0` | Requires read access to the target repository and its git history, a writable OS temp directory for the report, and a local opener (xdg-open, open, or start) plus a browser to view it. The report loads Tailwind and Mermaid from CDNs, so rendering it needs network access. Repository write access is not required; the workflow only recommends changes. |
