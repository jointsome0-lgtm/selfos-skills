...
No current skill uses the pairing. The four workflow skills that carried it (`grill-sdd`, `improve-codebase-architecture`, `slice`, `wayfinder`) were deliberately opened to model invocation on 2026-08-05 ([issue #100](https://github.com/jointsome0-lgtm/selfos-skills/issues/100)): Claude Code hides explicit-only skills from the model entirely — the agent cannot even propose them when a task matches — while Codex ignores the field, so the hosts diverged anyway. This is an intentional departure from the "user-invoked in both harnesses or neither" convention; do not restore the flags to make the hosts symmetric. Each of the four instead opens with a confirm-first prose gate: propose the workflow, start only after the owner confirms, and when running unattended record the recommendation without starting.
