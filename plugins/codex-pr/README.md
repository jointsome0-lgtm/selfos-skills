# `codex-pr@selfos` is deprecated

This legacy Claude package is a frozen compatibility snapshot. Its deprecation notice first shipped in version `0.3.1` on 2026-07-20.
...
/plugin update codex-pr@selfos
```

Install the canonical `watch` skill globally for Claude Code:

```bash
npx skills add jointsome0-lgtm/selfos-skills --skill watch --agent claude-code --global --yes
```

After confirming the canonical skill is available, remove the legacy package and reload plugins:

```text
/plugin uninstall codex-pr@selfos
/reload-plugins
```
