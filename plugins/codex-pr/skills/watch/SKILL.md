...
- Push everything a round should contain **before** opening the PR or re-triggering: a push racing the bot leaves the verdict on the older head. When its cutoff sits on a real round boundary (push event, a successfully posted `--trigger`, `--since`) the watcher surfaces such a stale-head review (exit 2 with a "reviewed commit ≠ expected head" warning) once no 👀 round is running — immediately if no round was observed, after a ~60 s verdict gap otherwise so an imminent same-head verdict wins — read the warning and re-check the findings against the current head before fixing. On the commit-date/start fallback cutoffs it stays silent about other heads and times out instead.
