...
/plugin update decision@selfos
```

Install the canonical `grilling` skill globally for Claude Code:

```bash
npx skills add jointsome0-lgtm/selfos-skills --skill grilling --agent claude-code --global --yes
```

After confirming the canonical skill is available, remove the legacy package and reload plugins:

```text
/plugin uninstall decision@selfos
/reload-plugins
```
