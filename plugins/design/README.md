...
/plugin update design@selfos
```

Install the canonical `codebase-design` and `deepen` skills globally for Claude Code:

```bash
npx skills add jointsome0-lgtm/selfos-skills --skill codebase-design deepen --agent claude-code --global --yes
```

After confirming the canonical skills are available, remove the legacy package and reload plugins:

```text
/plugin uninstall design@selfos
/reload-plugins
```
