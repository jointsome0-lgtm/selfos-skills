...
Write a single self-contained HTML file to the OS temporary directory — in this order, so nothing ever lands anywhere durable. First canonicalize the temp directory itself and verify it lies outside the repository worktree and any other durable, version-controlled, or synced location; a relative or symlinked temp root that resolves inside one is an error — stop and tell the user before anything is created. Only then create the file inside the verified directory through the platform's secure temp-file API (`tempfile.mkstemp`-style: atomic creation, unpredictable name, owner-only `0600` mode; prefix `architecture-review-`, suffix `.html`) — never a hand-assembled `<tmpdir>/<timestamp>.html` path — and write the content through the returned handle with a file-write API, never by interpolating it through a shell (no content-bearing heredocs). If any check fails after creation, delete the file before stopping. Each run gets a fresh file; nothing lands in the repository. Open it for the user without shell parsing: a direct argv invocation with the path as one literal argument (`xdg-open` on Linux, `open` on macOS), or the platform's file-association API on Windows (`os.startfile`-style — not the `start` shell built-in). Tell them the absolute path.
