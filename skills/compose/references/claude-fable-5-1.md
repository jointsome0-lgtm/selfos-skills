...
Replaces the "early stopping" snippet in the Fable 5 reference. On long asynchronous work it sometimes narrates the next step instead of doing it ("Next, I'll …") or asks permission for a step the request already covered ("Shall I apply this?"). Two system-prompt blocks together fix this; apply both, or only the first when length is tight. For autonomous runs, use the first in place of the core skill's compact autonomy policy. Its opening sentence carries much of the effect: keep it as written, and add product-specific confirmations in a sentence after it. Trade-off: it also asks less about ambiguous requests, so check that on your own tasks.
...
- **Extras and test sprawl**: on open-ended features it may fix nearby code, extend unmentioned behavior, or commit more test files than the change warrants. This instruction cuts both substantially with no measured loss in task success:
