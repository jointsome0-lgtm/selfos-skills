...
PINNED_DIGEST = "2c830687df9cca24ed7b15d606386598f5f87272be03195d92a0ad3092a9672c"
...
def bumped_template(directory: Path) -> Path:
    ...
    lines[0] = "<!-- sdd-conventions-template v1.3.0 -->"
...
class SyncConventionsTest(unittest.TestCase):
    ...
    def test_sync_inserts_block_and_check_passes(self) -> None:
        ...
        self.assertIn("<!-- BEGIN SDD-CONVENTIONS v1.2.0 sha256:", content)

        check = run(SCRIPT, "check", target)
        self.assertEqual(check.returncode, 0, check.stderr)
        self.assertIn("matches template v1.2.0", check.stdout)
    ...
    def test_check_detects_stale_after_template_update(self) -> None:
        ...
        self.assertIn("stale against template v1.3.0", check.stderr)
