...
When a task matches, announce that this workflow is starting and proceed — the owner can interrupt at any point. Unattended runs may work through the read-only exploration stage and write the report file, but must not open it — opening loads the CDN scripts into a page full of repository detail, a step the owner takes deliberately (weigh the compatibility warning for private repositories) — and they stop at every inner confirmation point: nothing lands in the repository without the owner. Surface architectural friction and propose **deepening opportunities** — refactors that turn shallow modules into deep ones. The aim is testability and AI-navigability.
