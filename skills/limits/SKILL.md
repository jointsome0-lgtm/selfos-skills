...
- Import gate: the only `<package>` import in `tests/` and `conftest.py` is `<package>.testing`, via `from` or an aliased `import`, since a bare `import <package>.testing` binds `<package>`. The dynamic forms `import_module`, `__import__`, and `importorskip` count as imports.
