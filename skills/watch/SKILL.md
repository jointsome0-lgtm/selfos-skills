...
2. Dispatch the named workflow with `gh workflow run <workflow-file> --repo <repo>` and the supplied inputs. A dispatched run executes the workflow definition from the ref it is given — the remote's default branch when `--ref` is omitted — so the trusted policy must govern the definition that runs, not only its name: prefer dispatching on the trusted default ref with the workflow reporting its named check on the supplied head SHA (inputs typically carry the PR number and the full SHA). Pass `--ref` for a branch whose tip is the verdict head only after verifying the workflow file at that ref is identical to the trusted base revision's — otherwise the unmerged change under review chooses the definition that runs with the repository's secrets.
3. Watch the run just dispatched, not a check name: identify the newly created run (for example via `gh run list --workflow <workflow-file> --repo <repo>` immediately after dispatching) and follow it with `gh run watch <run-id> --repo <repo>` — or, where the credential cannot use it (gh documents that fine-grained tokens cannot grant the `checks:read` it needs), track `gh run view <run-id> --repo <repo>` until the run completes. A pre-existing green check under the same name never satisfies the gate.
...
The gate does not change the merge step: the merge still passes `--match-head-commit <verdict-head>`, which is the same head the dispatch ran on. When the caller states that the repository's standing low-risk lane applies to the verdict head — for example a docs-only change the policy exempts — no dispatch is required and the preflight plus clean verdict remains sufficient.
