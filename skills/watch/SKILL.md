...
2. Dispatch the named workflow with `gh workflow run <workflow-file> --repo <repo>` and the supplied inputs. A dispatched run executes the workflow definition from the ref it is given — the remote's default branch when `--ref` is omitted — so the trusted policy must govern the definition that runs, not only its name: prefer dispatching the PR's trusted base ref (`--ref <base-branch>` — the same revision the policy was read from; omitting `--ref` runs the remote default branch's definition, which need not implement a non-default base's policy) with the workflow reporting its named check on the supplied head SHA (inputs typically carry the PR number and the full SHA). Pass `--ref` for a branch whose tip is the verdict head only after verifying the complete definition at that ref — the named workflow file plus every same-repository reusable workflow and local action it transitively references, which GitHub resolves from the same commit — is identical to the trusted base revision's; otherwise the unmerged change under review chooses the definition that runs with the repository's secrets.
