...
2. Dispatch the named workflow with `gh workflow run <workflow-file> --repo <repo>` and the supplied inputs. A dispatched run executes on the ref it is given — the remote's default branch when `--ref` is omitted — so the contract must state how the named check reaches the verdict head: either the workflow reports its check on the supplied head SHA (inputs typically carry the PR number and the full SHA), or the dispatch passes `--ref` for a branch whose tip is the verdict head.
3. Watch the run just dispatched, not a check name: identify the newly created run (for example via `gh run list --workflow <workflow-file> --repo <repo>` immediately after dispatching) and follow it with `gh run watch <run-id>`. A pre-existing green check under the same name never satisfies the gate.
