...
PINNED_DIGEST = "7a26bb18d2098dc60df6a072811bee6313a0d5520af4fb12b3225ef0c10adb69"
...
def bumped_template(directory: Path) -> Path:
    ...
    lines[0] = "<!-- sdd-conventions-template v1.4.0 -->"
...
class SyncConventionsTest(unittest.TestCase):
    ...
    def test_sync_inserts_block_and_check_passes(self) -> None:
        ...
        self.assertIn("<!-- BEGIN SDD-CONVENTIONS v1.3.0 sha256:", content)

        check = run(SCRIPT, "check", target)
        self.assertEqual(check.returncode, 0, check.stderr)
        self.assertIn("matches template v1.3.0", check.stdout)
    ...
    def test_check_detects_stale_after_template_update(self) -> None:
        ...
        self.assertIn("stale against template v1.4.0", check.stderr)
